 ...
 *
 * 사용법:
 *   node scripts/capture-image-queue.mjs           # 한 번 실행 후 종료
 *   node scripts/capture-image-queue.mjs --watch   # 계속 polling (10초 간격)
 */
...
// 동적 import
const { getPendingJobs, markJobAsRunning, markJobAsSuccess, markJobAsFailed } = await import('../src/lib/image-job.ts');
const { capturePostImage } = await import('../src/lib/capture-post-image.ts');
