...
## 로컬라이제이션 규칙
**필수 사항**: 서비스 내 사용자에게 표시되는 모든 문구는 반드시 다국어 처리되어야 함

### 자동 로컬라이제이션 프로토콜
1. **새 문구 추가 시 자동 처리**
   - 코드에 새로운 사용자 대면 텍스트를 추가할 때마다, 작업 완료 전에 반드시 다국어 파일에 추가
   - 하드코딩된 한국어/영어 텍스트를 발견하면 즉시 다국어화

2. **다국어 파일 위치 및 구조**
   - `src/i18n/en.ts` — 기본 인터페이스 및 영어 번역
   - `src/i18n/ko.ts` — 한국어 번역
   - `src/i18n/ja.ts` — 일본어 번역
   - `src/i18n/zh.ts` — 중국어 번역
...
4. **작업 완료 체크리스트**
   - [ ] 새 텍스트를 `src/i18n/en.ts`의 `Translations` 인터페이스에 키 추가
   - [ ] 모든 언어 파일 (en, ko, ja, zh)에 번역 추가
   - [ ] 컴포넌트에서 `useLanguage()` hook으로 번역 사용
   - [ ] 하드코딩된 텍스트 완전 제거

5. **번역 품질 기준**
   - 영어: 자연스러운 원어민 표현
   - 한국어: 존댓말 기본, 서비스 톤앤매너 유지
   - 일본어: 정중한 표현 (です・ます 체)
   - 중국어: 간체 기준, 자연스러운 현대 중국어
